...
bash "wildfly_extract" do
 code <<-EOH
  ...
  if [ -r /lib/lsb/init-functions ]; then
    cp #{wildfly_base}/#{ wildfly_targz.gsub('.tar.gz','') }/bin/init.d/wildfly-init-debian.sh /etc/init.d/#{wildfly_service}
    sed -i -e 's,NAME=wildfly,NAME='#{wildfly_service}',g' /etc/init.d/#{wildfly_service}
    sed -i -e 's,DEFAULT=/etc/default/$NAME,DEFAULT=/etc/default/#{wildfly_service}.conf,g' /etc/init.d/#{wildfly_service}
  fi
